...
draw.rectangle((0,0,width,height), outline=0, fill=0)
draw.text((x, top+0),  "Kig på", font=font, fill=255)
draw.text((x, top+30),"Kameraet", font=font, fill=255)
disp.image(image)
disp.display()
...
#headpose_manip.setWaitForConfigInput(True) #deprecated and replaced with following line
    ...
    while True:
        ...
        if msgs is not None:
            ...
            for i, detection in enumerate(dets):
                ...
                if name == 'Harald' and 100*conf >= 80:
                    print('Fundet!')                
        ...
        if cv2.waitKey(1) == ord('q'):
            draw.rectangle((0,0,width,height), outline=0, fill=0)
            draw.text((x, top+0),  "Scanning", font=font, fill=255)
            draw.text((x, top+30),"Komplet", font=font, fill=255)
            disp.image(image)
            disp.display()
            sleep(5)
            draw.rectangle((0,0,width,height), outline=0, fill=0)
